...
export default {
  data(){
    return {
              ...
              async created() {
                let course = await fetch("http://coursework3-env.eba-z4rfrmcm.eu-west-2.elasticbeanstalk.com/collection/lessons")
            ...
            methods: {
                filteredList(){
                    fetch(`http://coursework3-env.eba-z4rfrmcm.eu-west-2.elasticbeanstalk.com/collection/search/lessons?key_word=${this.search}`)
                    .then(response => {
                ...
                addToCart(lesson) {
                    this.lessons.find(item => item._id == lesson._id).space -= 1;
                    this.cart.push({ cart_id: (this.cart.length + 1), ...lesson });
                    // console.log('adding to cart', lesson.id)
                },
                removeFromCart(lesson) {
                    if (confirm('You are about to delete this!')) {
                        this.cart = [...this.cart].filter(item => item.cart_id != lesson.cart_id)
                    }
                },
                showCheckout() {
                ...
                submit(name,number) {
                    ...
                    fetch('http://coursework3-env.eba-z4rfrmcm.eu-west-2.elasticbeanstalk.com/collection/orders', {
                        method: 'POST',
                        headers: {
            ...
            computed: {
                ...
                sortedLessons() {
                    switch (this.type) {
                        ...
                        default:
                            return this.lessons.sort((a, b) => {
                                return this.sort == 'ascending' ? a._id - b._id : b._id - a._id;
                            })
                    }
                }
            },
